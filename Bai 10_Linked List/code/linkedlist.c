...
{
    int value;
    struct node* next; // con trỏ next vai trỏ lưu 1 mảng phần tử.
}node;

node* createNode(int value) // dùng con tro bởi bản chất node là mảng phần tử.
{
    node* ptr = (node*)malloc(sizeof(node)); // hàm cấp phát động malloc trả lại kiểu void nên phải ép kiểu (node*)
    ptr->value = value;
    ptr->next = NULL;
    return ptr;
}
